...
class ScriptedPolicy:
    """Two-phase controller: reposition behind block, then push toward target.

    Phase 1 (reposition): Move ee to approach point (90px behind block) while
        steering around the block so we don't push it sideways en route.
    Phase 2 (push): Once at approach, commit to pushing in push_dir for the
        remainder of the chunk.

    Approach distance of 90px (vs original 40px) ensures the ee is clear of
    the 45px contact radius before the push phase begins.
    """

    def __init__(self, speed=6.0, noise_std=0.3,
                 approach_dist=90, avoid_radius=60, push_thresh=30):
        self.speed        = speed
        self.noise_std    = noise_std
        self.approach_dist = approach_dist  # px behind block to stage from
        self.avoid_radius  = avoid_radius   # detour when this close to block
        self.push_thresh   = push_thresh    # enter push phase within this px of approach

    def get_actions(self, env, chunk_size=CHUNK_SIZE):
        ee     = np.array([env.ee_x,    env.ee_y],    dtype=float)
        block  = np.array([env.block_x, env.block_y], dtype=float)
        target = np.array([env.target_x, env.target_y], dtype=float)

        actions  = []
        pushing  = False   # once we commit to pushing in a chunk, keep going
        SUCCESS_DIST = 25  # stop pushing once block is this close to target
        ...
        for _ in range(chunk_size):
            ...
            if dist < SUCCESS_DIST:
                # Block is at target — hold ee in place so it doesn't nudge block
                action = np.zeros(2)
            else:
                push_dir = b2t / dist
                # Stage point: directly behind block relative to push direction
                approach = np.clip(block - push_dir * self.approach_dist, 30, 482)

                if pushing or np.linalg.norm(ee - approach) < self.push_thresh:
                    # Push phase — drive straight toward target
                    pushing = True
                    action  = push_dir * self.speed
                else:
                    # Reposition phase — move to approach while dodging block
                    d        = approach - ee
                    move_dir = d / max(np.linalg.norm(d), 1e-6)

                    # If the straight path to approach passes near the block,
                    # add a perpendicular offset to steer around it.
                    to_block = block - ee
                    along    = np.dot(to_block, move_dir)
                    if along > 0:
                        perp_dist = np.linalg.norm(to_block - along * move_dir)
                        if perp_dist < self.avoid_radius:
                            # Pick the perpendicular side that stays closer to approach
                            perp = np.array([-move_dir[1], move_dir[0]])
                            if (np.linalg.norm(ee + perp - approach) >
                                    np.linalg.norm(ee - perp - approach)):
                                perp = -perp
                            strength = (self.avoid_radius - perp_dist) / self.avoid_radius
                            move_dir = move_dir + perp * strength * 2.5
                            move_dir = move_dir / max(np.linalg.norm(move_dir), 1e-6)

                    action = move_dir * self.speed

            action = action + np.random.randn(2) * self.noise_std
            actions.append(action.copy())

            # Lightweight internal physics (keeps simulated ee/block in sync)
            ee = np.clip(ee + action, 15, 497)
            if np.linalg.norm(ee - block) < 45:
                block = np.clip(block + action * 0.15, 50, 462)
...
def run_demo(num_steps=200, num_vlm_layers=1, num_vit_layers=1,
             ...
             seed=0, checkpoint=None, scripted_actions=False):
    ...
    if checkpoint:
        ...
        from train.train import make_state_encoder, load_checkpoint
    ...
    state = env.reset()
    if scripted_actions:
        # Fixed aligned start: block directly above target, theta=0 → clean vertical push
        env.block_x = env.target_x
        env.block_y = env.target_y + 130
        env.block_theta = 0.0
        env.ee_x = env.target_x
        env.ee_y = env.target_y + 240
        state = env.get_state()
    frames = []
    timings = []
    action_buffer = []
    chunk_idx = 0
    last_ms = 0.0
    scripted_policy = ScriptedPolicy(speed=5.0, noise_std=0.0) if scripted_actions else None

    print(f"\nRunning demo: {num_steps} steps{'  [scripted actions]' if scripted_actions else ''}...")
    for step in range(num_steps):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--scripted_actions", action="store_true",
                        help="Use scripted policy for actions (VLA only timed, not executed)")
