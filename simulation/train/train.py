...
def train(total_steps=10000, lr=1e-3, batch_size=32, save_every=1000,
    ...
    t_pos = torch.linspace(0, 1, CHUNK_SIZE).unsqueeze(1)  # [32, 1], reused each step

    for step in range(start_step, total_steps):
        # Collect batch of (state, gt_actions) pairs from env
        ...
        for _ in range(batch_size):
            ...
            targets_list.append(policy.get_actions(env))  # [32, 2]
            states_list.append(env.get_state())           # [5]

        states_t  = torch.tensor(np.stack(states_list),  dtype=torch.float32)  # [B, 5]
        targets_t = torch.tensor(np.stack(targets_list), dtype=torch.float32)  # [B, 32, 2]

        # Single batched forward: [B, 768] → [B, 32, 768] → [B, 32, 2]
        act_emb   = state_encoder(states_t)                                     # [B, 768]
        act_input = act_emb.unsqueeze(1).expand(-1, CHUNK_SIZE, -1) + t_pos.unsqueeze(0) * 0.1  # [B, 32, 768]
        act_out   = model.exp_self(act_input)                                   # [B, 32, 768]
        pred      = model.post_proj(model.post_norm(act_out))                   # [B, 32, 32]
        pred_2d   = pred[:, :, :ACTION_DIM]                                     # [B, 32, 2]

        loss = nn.functional.mse_loss(pred_2d, targets_t)
