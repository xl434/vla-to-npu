...
// Uses aie::mul(num, aie::inv(den)) instead of aie::div(num, den) to avoid
// register-sourcing crash when denominator comes from aie::add chain.
...
get_tanh_vec(const aie::vector<T_in, vec_factor>& x) {
  ...
  // This avoids SRS-register sourcing crash
