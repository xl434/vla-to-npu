...
int main(int argc, const char *argv[]) {
  ...
  auto bo_in0 = xrt::bo(device, 12288 * sizeof(float),
  ...
  if (ifile0_size != 12288 * sizeof(float)) {
    std::cerr << "Error: Invalid input file, byte number mismatch.\n";
    return 1;
  }
  std::vector<float> vec0(12288);
  ifile0.read(reinterpret_cast<char*>(vec0.data()), 12288 * sizeof(float));
  srcVec0.insert(srcVec0.end(), vec0.begin(), vec0.end());
  memcpy(bufIn0, srcVec0.data(), (srcVec0.size() * sizeof(float)));
  auto bo_in1 = xrt::bo(device, 2 * sizeof(int),
  ...
  if (ifile1_size != 2 * sizeof(int)) {
    std::cerr << "Error: Invalid input file, byte number mismatch.\n";
    return 1;
  }
  std::vector<int> vec1(2);
  ifile1.read(reinterpret_cast<char*>(vec1.data()), 2 * sizeof(int));
  srcVec1.insert(srcVec1.end(), vec1.begin(), vec1.end());
  memcpy(bufIn1, srcVec1.data(), (srcVec1.size() * sizeof(int)));

  auto bo_out2 = xrt::bo(device, 12288 * sizeof(float),
  ...
  float *bufOut2 = bo_out2.map<float *>();
  std::ofstream ofile2("output2.data", std::ios::binary);
  ofile2.write(reinterpret_cast<const char*>(bufOut2 + 0), 12288 * sizeof(float));
  ofile2.close();
  if (trace_size > 0) {
