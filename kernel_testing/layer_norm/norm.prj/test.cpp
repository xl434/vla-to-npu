...
int main(int argc, const char *argv[]) {
  ...
  auto bo_in0 = xrt::bo(device, 13056 * sizeof(float),
  ...
  if (ifile2_size != 768 * sizeof(float)) {
    std::cerr << "Error: Invalid input file, byte number mismatch.\n";
    return 1;
  }
  std::vector<float> vec2(768);
  ifile2.read(reinterpret_cast<char*>(vec2.data()), 768 * sizeof(float));
  srcVec0.insert(srcVec0.end(), vec2.begin(), vec2.end());
  memcpy(bufIn0, srcVec0.data(), (srcVec0.size() * sizeof(float)));

  auto bo_out1 = xrt::bo(device, 12288 * sizeof(float),
                        XRT_BO_FLAGS_HOST_ONLY, kernel.group_id(4));
  auto bo_in2 = xrt::bo(device, 768 * sizeof(float),
  ...
  if (ifile1_size != 768 * sizeof(float)) {
    std::cerr << "Error: Invalid input file, byte number mismatch.\n";
    return 1;
  }
  std::vector<float> vec1(768);
  ifile1.read(reinterpret_cast<char*>(vec1.data()), 768 * sizeof(float));
  ...
  ofile3.write(reinterpret_cast<const char*>(bufOut1 + 0), 12288 * sizeof(float));
  ofile3.close();
  ifile1.close();
  if (trace_size > 0) {
