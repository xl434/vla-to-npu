...
int main(int argc, const char *argv[]) {
  ...
  auto bo_out1 = xrt::bo(device, 16 * sizeof(float),
                        XRT_BO_FLAGS_HOST_ONLY, kernel.group_id(4));
  auto bo_in2 = xrt::bo(device, 256 * sizeof(float),
  ...
  if (ifile1_size != 256 * sizeof(float)) {
    std::cerr << "Error: Invalid input file, byte number mismatch.\n";
    return 1;
  }
  std::vector<float> vec1(256);
  ifile1.read(reinterpret_cast<char*>(vec1.data()), 256 * sizeof(float));
  ...
  ofile2.write(reinterpret_cast<const char*>(bufOut1 + 0), 16 * sizeof(float));
  ofile2.close();
  ifile1.close();
  if (trace_size > 0) {
