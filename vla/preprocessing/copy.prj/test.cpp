...
int main(int argc, const char *argv[]) {
  ...
  auto bo_in0 = xrt::bo(device, 256 * sizeof(std::bfloat16_t),
  ...
  if (ifile0_size != 256 * sizeof(std::bfloat16_t)) {
    std::cerr << "Error: Invalid input file, byte number mismatch.\n";
    return 1;
  }
  std::vector<std::bfloat16_t> vec0(256);
  ifile0.read(reinterpret_cast<char*>(vec0.data()), 256 * sizeof(std::bfloat16_t));
  ...
  ofile1.write(reinterpret_cast<const char*>(bufOut1 + 0), 256 * sizeof(std::bfloat16_t));
  ofile1.close();
  if (trace_size > 0) {
