...
int main(int argc, const char *argv[]) {
  ...
  auto bo_in0 = xrt::bo(device, 64 * sizeof(float),
  ...
  if (ifile0_size != 64 * sizeof(float)) {
    std::cerr << "Error: Invalid input file, byte number mismatch.\n";
    return 1;
  }
  std::vector<float> vec0(64);
  ifile0.read(reinterpret_cast<char*>(vec0.data()), 64 * sizeof(float));
  ...
  if (ifile1_size != 32 * sizeof(float)) {
    std::cerr << "Error: Invalid input file, byte number mismatch.\n";
    return 1;
  }
  std::vector<float> vec1(32);
  ifile1.read(reinterpret_cast<char*>(vec1.data()), 32 * sizeof(float));
