...
int main(int argc, const char *argv[]) {
  ...
  auto bo_in0 = xrt::bo(device, 10240 * sizeof(std::bfloat16_t),
  ...
  if (ifile0_size != 2048 * sizeof(std::bfloat16_t)) {
    std::cerr << "Error: Invalid input file, byte number mismatch.\n";
    return 1;
  }
  std::vector<std::bfloat16_t> vec0(2048);
  ifile0.read(reinterpret_cast<char*>(vec0.data()), 2048 * sizeof(std::bfloat16_t));
  ...
  if (ifile2_size != 8192 * sizeof(std::bfloat16_t)) {
    std::cerr << "Error: Invalid input file, byte number mismatch.\n";
    return 1;
  }
  std::vector<std::bfloat16_t> vec2(8192);
  ifile2.read(reinterpret_cast<char*>(vec2.data()), 8192 * sizeof(std::bfloat16_t));
  srcVec0.insert(srcVec0.end(), vec2.begin(), vec2.end());
  memcpy(bufIn0, srcVec0.data(), (srcVec0.size() * sizeof(std::bfloat16_t)));

  auto bo_out1 = xrt::bo(device, 2048 * sizeof(std::bfloat16_t),
  ...
  if (ifile1_size != 8192 * sizeof(std::bfloat16_t)) {
    std::cerr << "Error: Invalid input file, byte number mismatch.\n";
    return 1;
  }
  std::vector<std::bfloat16_t> vec1(8192);
  ifile1.read(reinterpret_cast<char*>(vec1.data()), 8192 * sizeof(std::bfloat16_t));
  ...
  ofile3.write(reinterpret_cast<const char*>(bufOut1 + 0), 2048 * sizeof(std::bfloat16_t));
  ofile3.close();
  ifile1.close();
  if (trace_size > 0) {
