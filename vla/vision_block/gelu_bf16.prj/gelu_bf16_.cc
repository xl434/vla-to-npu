...
// Tanh LUT in float32 for precision (same as gelu.cc)
constexpr float TANH_MAX_X = 4.0f;
constexpr int TANH_LUT_SIZE = 128;
...
// bf16 GELU: load bf16, compute in float32 (for LUT precision), store bf16
void gelu_bf16(bfloat16 input_x[4][768], bfloat16 output_x[4][768]) {
  ...
  for (int s = 0; s < SEQ_TILE; ++s) {
    ...
    for (int i = 0; i < FEATURE_DIM_TILE; ++i) {
      ...
      float t = get_tanh(inner);
      output_ptr[i] = (bfloat16)((t + 1.0f) * x * 0.5f);
    }
  }
}

// 8-row variant: same per-element logic, double the SEQ tile.
// SRAM: 8×768×2 = 12 KB input + 12 KB output = 24 KB (single buffer);
// double-buffered ≈ 48 KB + ~5 KB program < 64 KB ✓
