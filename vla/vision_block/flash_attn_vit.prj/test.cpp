...
int main(int argc, const char *argv[]) {
  ...
  if (ifile0_size != 65536 * sizeof(std::bfloat16_t)) {
    std::cerr << "Error: Invalid input file, byte number mismatch.\n";
    return 1;
  }
  std::vector<std::bfloat16_t> vec0(65536);
  ifile0.read(reinterpret_cast<char*>(vec0.data()), 65536 * sizeof(std::bfloat16_t));
  ...
  if (ifile2_size != 65536 * sizeof(std::bfloat16_t)) {
    std::cerr << "Error: Invalid input file, byte number mismatch.\n";
    return 1;
  }
  std::vector<std::bfloat16_t> vec2(65536);
  ifile2.read(reinterpret_cast<char*>(vec2.data()), 65536 * sizeof(std::bfloat16_t));
  ...
  auto bo_in2 = xrt::bo(device, 65536 * sizeof(std::bfloat16_t),
  ...
  if (ifile1_size != 65536 * sizeof(std::bfloat16_t)) {
    std::cerr << "Error: Invalid input file, byte number mismatch.\n";
    return 1;
  }
  std::vector<std::bfloat16_t> vec1(65536);
  ifile1.read(reinterpret_cast<char*>(vec1.data()), 65536 * sizeof(std::bfloat16_t));
  ...
  ofile3.write(reinterpret_cast<const char*>(bufOut1 + 0), 65536 * sizeof(std::bfloat16_t));
  ofile3.close();
  ifile1.close();
  if (trace_size > 0) {
