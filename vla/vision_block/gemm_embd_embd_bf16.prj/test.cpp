...
int main(int argc, const char *argv[]) {
  ...
  auto bo_out1 = xrt::bo(device, 786432 * sizeof(std::bfloat16_t),
                        XRT_BO_FLAGS_HOST_ONLY, kernel.group_id(4));
  auto bo_in2 = xrt::bo(device, 589824 * sizeof(std::bfloat16_t),
  ...
  if (ifile1_size != 589824 * sizeof(std::bfloat16_t)) {
    std::cerr << "Error: Invalid input file, byte number mismatch.\n";
    return 1;
  }
  std::vector<std::bfloat16_t> vec1(589824);
  ifile1.read(reinterpret_cast<char*>(vec1.data()), 589824 * sizeof(std::bfloat16_t));
  ...
  ofile2.write(reinterpret_cast<const char*>(bufOut1 + 0), 786432 * sizeof(std::bfloat16_t));
  ofile2.close();
  ifile1.close();
  if (trace_size > 0) {
