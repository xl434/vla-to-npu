...
int main(int argc, const char *argv[]) {
  ...
  auto bo_in0 = xrt::bo(device, 32768 * sizeof(float),
  ...
  if (ifile0_size != 32768 * sizeof(float)) {
    std::cerr << "Error: Invalid input file, byte number mismatch.\n";
    return 1;
  }
  std::vector<float> vec0(32768);
  ifile0.read(reinterpret_cast<char*>(vec0.data()), 32768 * sizeof(float));
  srcVec0.insert(srcVec0.end(), vec0.begin(), vec0.end());
  memcpy(bufIn0, srcVec0.data(), (srcVec0.size() * sizeof(float)));

  auto bo_out1 = xrt::bo(device, 32768 * sizeof(float),
  ...
  ofile1.write(reinterpret_cast<const char*>(bufOut1 + 0), 32768 * sizeof(float));
  ofile1.close();
  if (trace_size > 0) {
