...
int main(int argc, const char *argv[]) {
  ...
  auto bo_in0 = xrt::bo(device, 25344 * sizeof(std::bfloat16_t),
  ...
  if (ifile0_size != 24576 * sizeof(std::bfloat16_t)) {
    std::cerr << "Error: Invalid input file, byte number mismatch.\n";
    return 1;
  }
  std::vector<std::bfloat16_t> vec0(24576);
  ifile0.read(reinterpret_cast<char*>(vec0.data()), 24576 * sizeof(std::bfloat16_t));
  ...
  if (ifile2_size != 768 * sizeof(std::bfloat16_t)) {
    std::cerr << "Error: Invalid input file, byte number mismatch.\n";
    return 1;
  }
  std::vector<std::bfloat16_t> vec2(768);
  ifile2.read(reinterpret_cast<char*>(vec2.data()), 768 * sizeof(std::bfloat16_t));
  srcVec0.insert(srcVec0.end(), vec2.begin(), vec2.end());
  memcpy(bufIn0, srcVec0.data(), (srcVec0.size() * sizeof(std::bfloat16_t)));

  auto bo_out1 = xrt::bo(device, 24576 * sizeof(std::bfloat16_t),
                        XRT_BO_FLAGS_HOST_ONLY, kernel.group_id(4));
  auto bo_in2 = xrt::bo(device, 768 * sizeof(std::bfloat16_t),
  ...
  if (ifile1_size != 768 * sizeof(std::bfloat16_t)) {
    std::cerr << "Error: Invalid input file, byte number mismatch.\n";
    return 1;
  }
  std::vector<std::bfloat16_t> vec1(768);
  ifile1.read(reinterpret_cast<char*>(vec1.data()), 768 * sizeof(std::bfloat16_t));
  ...
  std::ofstream ofile3("output3.data", std::ios::binary);
  ofile3.write(reinterpret_cast<const char*>(bufOut1 + 0), 24576 * sizeof(std::bfloat16_t));
  ofile3.close();
  ifile1.close();
  if (trace_size > 0) {
