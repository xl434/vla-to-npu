...
int main(int argc, const char *argv[]) {
  ...
  auto bo_in0 = xrt::bo(device, 10240 * sizeof(float),
  ...
  if (ifile0_size != 10240 * sizeof(float)) {
    std::cerr << "Error: Invalid input file, byte number mismatch.\n";
    return 1;
  }
  std::vector<float> vec0(10240);
  ifile0.read(reinterpret_cast<char*>(vec0.data()), 10240 * sizeof(float));
  srcVec0.insert(srcVec0.end(), vec0.begin(), vec0.end());
  memcpy(bufIn0, srcVec0.data(), (srcVec0.size() * sizeof(float)));

  auto bo_out1 = xrt::bo(device, 10240 * sizeof(float),
                        XRT_BO_FLAGS_HOST_ONLY, kernel.group_id(4));
  auto bo_in2 = xrt::bo(device, 10240 * sizeof(float),
  ...
  if (ifile1_size != 10240 * sizeof(float)) {
    std::cerr << "Error: Invalid input file, byte number mismatch.\n";
    return 1;
  }
  std::vector<float> vec1(10240);
  ifile1.read(reinterpret_cast<char*>(vec1.data()), 10240 * sizeof(float));
  ...
  ofile2.write(reinterpret_cast<const char*>(bufOut1 + 0), 10240 * sizeof(float));
  ofile2.close();
  ifile1.close();
  if (trace_size > 0) {
