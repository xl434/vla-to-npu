...
int main(int argc, const char *argv[]) {
  ...
  auto bo_in0 = xrt::bo(device, 2048 * sizeof(float),
  ...
  if (ifile0_size != 2048 * sizeof(float)) {
    std::cerr << "Error: Invalid input file, byte number mismatch.\n";
    return 1;
  }
  std::vector<float> vec0(2048);
  ifile0.read(reinterpret_cast<char*>(vec0.data()), 2048 * sizeof(float));
  ...
  auto bo_in2 = xrt::bo(device, 2048 * sizeof(float),
  ...
  if (ifile1_size != 2048 * sizeof(float)) {
    std::cerr << "Error: Invalid input file, byte number mismatch.\n";
    return 1;
  }
  std::vector<float> vec1(2048);
  ifile1.read(reinterpret_cast<char*>(vec1.data()), 2048 * sizeof(float));
  ...
  ofile2.write(reinterpret_cast<const char*>(bufOut1 + 0), 2048 * sizeof(float));
  ofile2.close();
  ifile1.close();
  if (trace_size > 0) {
