...
int main(int argc, const char *argv[]) {
  ...
  auto bo_in0 = xrt::bo(device, 1024 * sizeof(std::bfloat16_t),
  ...
  if (ifile0_size != 1024 * sizeof(std::bfloat16_t)) {
    std::cerr << "Error: Invalid input file, byte number mismatch.\n";
    return 1;
  }
  std::vector<std::bfloat16_t> vec0(1024);
  ifile0.read(reinterpret_cast<char*>(vec0.data()), 1024 * sizeof(std::bfloat16_t));
  srcVec0.insert(srcVec0.end(), vec0.begin(), vec0.end());
  memcpy(bufIn0, srcVec0.data(), (srcVec0.size() * sizeof(std::bfloat16_t)));
  auto bo_in1 = xrt::bo(device, 1 * sizeof(int),
                        XRT_BO_FLAGS_HOST_ONLY, kernel.group_id(4));
  int *bufIn1 = bo_in1.map<int *>();
  std::vector<int> srcVec1;
  ...
  if (ifile1_size != 1 * sizeof(int)) {
    std::cerr << "Error: Invalid input file, byte number mismatch.\n";
    return 1;
  }
  std::vector<int> vec1(1);
  ifile1.read(reinterpret_cast<char*>(vec1.data()), 1 * sizeof(int));
  srcVec1.insert(srcVec1.end(), vec1.begin(), vec1.end());
  memcpy(bufIn1, srcVec1.data(), (srcVec1.size() * sizeof(int)));

  auto bo_out2 = xrt::bo(device, 1024 * sizeof(std::bfloat16_t),
  ...
  if (!do_profile) {
    ...
    auto run = kernel(opcode, bo_instr, instr_v.size(), bo_in0, bo_in1, bo_out2, bo_trace);
    ...
    for (size_t i = 0; i < n_test_iterations; i++) {
        auto start = std::chrono::high_resolution_clock::now();
        auto run = kernel(opcode, bo_instr, instr_v.size(), bo_in0, bo_in1, bo_out2);
        run.wait();
  ...
  std::bfloat16_t *bufOut2 = bo_out2.map<std::bfloat16_t *>();
  std::ofstream ofile2("output2.data", std::ios::binary);
  ofile2.write(reinterpret_cast<const char*>(bufOut2 + 0), 1024 * sizeof(std::bfloat16_t));
  ofile2.close();
  if (trace_size > 0) {
