# Mixed-precision GEMM module: supports bf16 input with float32 output
# Uses implicit type casting on array assignment (bf16 → f32) to work around
# allo's type inference limitation with allo.add on mixed types.
...
def GEMM(M, N, K, Pm, Pn, Pk, TyI, TyO, col_num=4, row_num=4):
    ...
    def top(A: TyI[M, K], B: TyI[K, N], C: TyO[M, N]):
        ...
        def gemm(
            ...
            with allo.meta_else():
                C_in[:, :] = 0
            # Matmul in input type (bf16)
            matmul_result: TyI[Mt, Nt] = allo.matmul(local_A, local_B)
            # Implicit cast: bf16 → f32 via array assignment
            matmul_f32: TyO[Mt, Nt]
            matmul_f32[:, :] = matmul_result[:, :]
            # Add in output type (f32 + f32 = no type conflict)
