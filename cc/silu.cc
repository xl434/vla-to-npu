...
//     y[lane] = get_sigmoid_approx(x[lane]);
//     y[lane + 1] = get_sigmoid_approx(x[lane + 1]);
//     y[lane + 2] = get_sigmoid_approx(x[lane + 2]);
//     y[lane + 3] = get_sigmoid_approx(x[lane + 3]);
//     y[lane + 4] = get_sigmoid_approx(x[lane + 4]);
//     y[lane + 5] = get_sigmoid_approx(x[lane + 5]);
//     y[lane + 6] = get_sigmoid_approx(x[lane + 6]);
//     y[lane + 7] = get_sigmoid_approx(x[lane + 7]);
//   }
//   return y;
// }
...
// // TAYLOR APPROX. -----------------------------------
// template<int vec_factor>
// static inline aie::vector<float, vec_factor> get_sigmoid_approx_vec(const aie::vector<float, vec_factor>& x) {
//   using vec_t = aie::vector<float, vec_factor>;

//   const vec_t c0_5 = aie::broadcast<float, vec_factor>(0.5f);
//   const vec_t c0_25 = aie::broadcast<float, vec_factor>(0.25f);
//   const vec_t cdiv48 = aie::broadcast<float, vec_factor>(0.02083333333f);
//   const vec_t cdiv480 = aie::broadcast<float, vec_factor>(0.002083333333f);
//   const vec_t c17 = aie::broadcast<float, vec_factor>(17.0f);
//   const vec_t cdiv80640 = aie::broadcast<float, vec_factor>(0.00001240079365f);
//   const vec_t c31 = aie::broadcast<float, vec_factor>(31.0f);
//   const vec_t cdiv1451520= aie::broadcast<float, vec_factor>(0.0000006889329806f);

//   // Taylor: 1/2 + x/4 - x^3/48
//   vec_t x1div4         = aie::mul(x, c0_25);          // x/4
//   vec_t xtemp          = aie::mul(x, x);              // x^2
//   vec_t x3             = aie::mul(xtemp, x);          // x^3
//   vec_t negx3div48     = aie::negmul(x3, cdiv48);     // -x^3/48
//   xtemp                = aie::mul(x3, x);             // x^4
//   vec_t x5             = aie::mul(xtemp, x);          // x^5
//   vec_t x5div480       = aie::mul(x5, cdiv480);       // x^5/480
//   xtemp                = aie::mul(x5, x);             // x^6
//   vec_t x7             = aie::mul(xtemp, x);          // x^7
//   vec_t x7div80640     = aie::negmul(x7, cdiv80640);  // -x^7/80640
//   vec_t x717div80640   = aie::mul(x7div80640, c17);   // -17x^7/80640
//   xtemp                = aie::mul(x7, x);             // x^8
//   vec_t x9             = aie::mul(xtemp, x);          // x^9
//   vec_t x9div1451520   = aie::mul(x9, cdiv1451520);   // x^9/1451520
//   vec_t x931div1451520 = aie::mul(x9div1451520, c31);   // 31x^9/1451520

//   xtemp = aie::add(x1div4, negx3div48);
//   xtemp = aie::add(xtemp, x5div480);
//   xtemp = aie::add(xtemp, x717div80640);
//   xtemp = aie::add(xtemp, x931div1451520);

//   // vec_t r = aie::add(c0_5, x1div4);
//   vec_t r = aie::add(c0_5, xtemp);
...
// void silu_float32(T_in input_x[4][768], T_out output_x[4][768]) {
//   event0();
//   constexpr int SEQ_TILE = 4;
//   constexpr int FEATURE_DIM_TILE = 768;
//   constexpr int vec_factor = 32;
//   using vec_t = aie::vector<T_out, vec_factor>;

//   const vec_t neg5 = aie::broadcast<float, vec_factor>(-5.0f);
//   const vec_t pos5 = aie::broadcast<float, vec_factor>(5.0f);
//   const vec_t one = aie::broadcast<float, vec_factor>(1.0f);

//   for (int iter = 0; iter < SEQ_TILE; iter++) {
//     T_in *__restrict input_ptr = &input_x[iter][0];
//     T_in *__restrict output_ptr = &output_x[iter][0];
...
//       vec_t zeros = aie::zeros<float, vec_factor>();

//       auto m_ltneg5 = aie::lt(x, neg5);
//       sigmoid = aie::select(sigmoid, zeros, m_ltneg5);  // x < -5 → sigmoid = 0

//       auto m_ge5 = aie::ge(x, pos5);
//       sigmoid = aie::select(sigmoid, one, m_ge5);       // x >= 5 → sigmoid = 1

//       vec_t y = aie::mul(x, sigmoid).template to_vector<float>();
...
template <int vec_factor>
aie::vector<float, vec_factor> linear_approx(aie::vector<float, vec_factor> &x) {
  using vec_t = aie::vector<float, vec_factor>;
  vec_t c0_0041 = aie::broadcast<float, vec_factor>(0.0041f);
  vec_t c0_9736 = aie::broadcast<float, vec_factor>(0.9736f);
...
void silu_bfloat16(bfloat16 input_x[4][768], bfloat16 output_x[4][768]) {
  ...
  for (int s = 0; s < SEQ_TILE; ++s) {
    ...
    for (int inner_it = 0; inner_it < FEATURE_DIM_TILE; inner_it += vec_factor) {
      ...
      // Taylor series
      fvec_t negx        = aie::negmul(x, one);
      fvec_t x2          = aie::mul(x, x);
      fvec_t x2fiv2fac   = aie::mul(x2, c2divfac);
      fvec_t x3          = aie::mul(x2, x);
      fvec_t x3div3fac   = aie::negmul(x3, c3divfac);
      fvec_t x4          = aie::mul(x3, x);
      fvec_t x4div4fac   = aie::mul(x4, c4divfac);
      fvec_t x5          = aie::mul(x4, x);
      fvec_t x5div5fac   = aie::negmul(x5, c5divfac);
      fvec_t x6          = aie::mul(x5, x);
      fvec_t x6div6fac   = aie::mul(x6, c6divfac);
      fvec_t x7          = aie::mul(x6, x);
      fvec_t x7div7fac   = aie::negmul(x7, c7divfac);
      fvec_t x8          = aie::mul(x7, x);
      fvec_t x8div8fac   = aie::mul(x8, c8divfac);
      fvec_t x9          = aie::mul(x8, x);
      fvec_t x9div9fac   = aie::negmul(x9, c9divfac);
      fvec_t x10         = aie::mul(x9, x);
      fvec_t x10div10fac = aie::mul(x10, c10divfac);
      fvec_t x11         = aie::mul(x10, x);
      fvec_t x11div11fac = aie::negmul(x11, c11divfac);
      fvec_t x12         = aie::mul(x11, x);
      fvec_t x12div12fac = aie::mul(x12, c12divfac);
      fvec_t x13         = aie::mul(x12, x);
      fvec_t x13div13fac = aie::negmul(x13, c13divfac);
      fvec_t x14         = aie::mul(x13, x);
      fvec_t x14div14fac = aie::mul(x14, c14divfac);
      fvec_t x15         = aie::mul(x14, x);
      fvec_t x15div15fac = aie::negmul(x15, c15divfac);
      fvec_t x16         = aie::mul(x15, x);
      fvec_t x16div16fac = aie::mul(x16, c16divfac);

      fvec_t xtemp = aie::add(negx, x2fiv2fac);
      xtemp = aie::add(xtemp, x3div3fac);
      xtemp = aie::add(xtemp, x4div4fac);
      xtemp = aie::add(xtemp, x5div5fac);
      xtemp = aie::add(xtemp, x6div6fac);
      xtemp = aie::add(xtemp, x7div7fac);
      xtemp = aie::add(xtemp, x8div8fac);
      xtemp = aie::add(xtemp, x9div9fac);
      xtemp = aie::add(xtemp, x10div10fac);
      xtemp = aie::add(xtemp, x11div11fac);
      xtemp = aie::add(xtemp, x12div12fac);
      xtemp = aie::add(xtemp, x13div13fac);
      xtemp = aie::add(xtemp, x14div14fac);
      xtemp = aie::add(xtemp, x15div15fac);
      xtemp = aie::add(xtemp, x16div16fac);

      fvec_t expnegx = aie::add(one, xtemp);

      fvec_t sigmoid = aie::div(one, aie::add(one, expnegx));
      fvec_t linear  = linear_approx<vec_factor>(x);                 // linear region (4 < x < 7)
...
void silu(bfloat16 input_x[4][768], bfloat16 output_x[4][768]) {
// silu_bfloat16<bfloat16, bfloat16>(input_x, output_x);
