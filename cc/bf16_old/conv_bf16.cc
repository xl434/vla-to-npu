...
{
    void conv(
        bfloat16 input[64][64],
        bfloat16 kernel[16][16],
        bfloat16 output[4][4])
    {
        ...
        for (int ph = 0; ph < NUM_PATCHES_H; ++ph)
        {
            ...
            {
                bfloat16 *__restrict output_ptr = &output[ph][pw];
                bfloat16 *__restrict input_ptr0 = &input[ph * PATCH_SIZE + 0][pw * PATCH_SIZE];
                bfloat16 *__restrict kernel_ptr0 = &kernel[0][0];
                ...
                {
                    bfloat16 *__restrict input_ptr = &input[ph * PATCH_SIZE + kh][pw * PATCH_SIZE];
                    bfloat16 *__restrict kernel_ptr = &kernel[kh][0];
