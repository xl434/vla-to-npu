 ...
 *
 * Masked (causal) softmax float32 for 128-column rows
 * (SmolVLA text encoder: attention scores are [128, 128])
 */
...
void masked_softmax_128_float32(float attention_score[8][128],
                                int tile_row_start[1],
                                float attn_weights[8][128]) {
  ...
  for (int r = 0; r < TILE_ROWS; ++r) {
    ...
    float *__restrict score_ptr = &attention_score[r][0];
    float *__restrict weight_ptr = &attn_weights[r][0];
    ...
    // Load 4 vectors of 32 elements = 128 columns
    aie::vector<float, VEC_SIZE> scores_v0 = aie::load_v<VEC_SIZE>(score_ptr);
    aie::vector<float, VEC_SIZE> scores_v1 = aie::load_v<VEC_SIZE>(score_ptr + VEC_SIZE);
    aie::vector<float, VEC_SIZE> scores_v2 = aie::load_v<VEC_SIZE>(score_ptr + 2 * VEC_SIZE);
    aie::vector<float, VEC_SIZE> scores_v3 = aie::load_v<VEC_SIZE>(score_ptr + 3 * VEC_SIZE);

    // Find max for numerical stability
    float row_max = aie::reduce_max(scores_v0);
    row_max = std::max(row_max, aie::reduce_max(scores_v1));
    row_max = std::max(row_max, aie::reduce_max(scores_v2));
    row_max = std::max(row_max, aie::reduce_max(scores_v3));

    scores_v0 = aie::add(scores_v0, -row_max);
    scores_v1 = aie::add(scores_v1, -row_max);
    scores_v2 = aie::add(scores_v2, -row_max);
    scores_v3 = aie::add(scores_v3, -row_max);
    ...
    for (int k = 0; k < VEC_SIZE; ++k) {
      float exp_value = get_exp(scores_v2[k]);
      sum_exp += exp_value;
      attn_weights[r][2 * VEC_SIZE + k] = exp_value;
    }
    for (int k = 0; k < VEC_SIZE; ++k) {
      float exp_value = get_exp(scores_v3[k]);
      sum_exp += exp_value;
      attn_weights[r][3 * VEC_SIZE + k] = exp_value;
    }

    // Normalize
    float scale = 1.0f / sum_exp;
    aie::vector<float, VEC_SIZE> w0 = aie::load_v<VEC_SIZE>(weight_ptr);
    aie::vector<float, VEC_SIZE> w1 = aie::load_v<VEC_SIZE>(weight_ptr + VEC_SIZE);
    aie::vector<float, VEC_SIZE> w2 = aie::load_v<VEC_SIZE>(weight_ptr + 2 * VEC_SIZE);
    aie::vector<float, VEC_SIZE> w3 = aie::load_v<VEC_SIZE>(weight_ptr + 3 * VEC_SIZE);
    w0 = aie::mul(w0, scale);
    w1 = aie::mul(w1, scale);
    w2 = aie::mul(w2, scale);
    w3 = aie::mul(w3, scale);
    aie::store_v(weight_ptr, w0);
    aie::store_v(weight_ptr + VEC_SIZE, w1);
    aie::store_v(weight_ptr + 2 * VEC_SIZE, w2);
    aie::store_v(weight_ptr + 3 * VEC_SIZE, w3);
  }
}
