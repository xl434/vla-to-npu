...
// Combined sin+cos: avoids the memref.copy bug in Allo when a single-input
// single-output ExternalModule has identical input and output shapes.
void sin_cos_float32(
    float in_mat[32][64],
    float out_mat[64][64]
) {
    // First 32 rows store sin
    sin_f32(in_mat, out_mat);

    // Next 32 rows store cos
    cos_kernel_32x64(in_mat, &out_mat[32]);
}
// void sin_cos_float32(
//     float in_mat[64][64],
//     float out_mat[128][64]
// ) {
//     // First 64 rows store sin
//     sin_f32(in_mat, out_mat);

//     // Next 64 rows store cos
//     cos_kernel_32x64(in_mat, &out_mat[64]);
// }
