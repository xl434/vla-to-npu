...
aie::vector<float, vec_factor> linear_approx_f32(aie::vector<float, vec_factor> &x) {
  using fvec_t = aie::vector<float, vec_factor>;
  fvec_t c0_0041 = aie::broadcast<float, vec_factor>(0.0041f);
  fvec_t c0_9736 = aie::broadcast<float, vec_factor>(0.9736f);
...
void silu_bfloat16_160(bfloat16 input_x[4][160], bfloat16 output_x[4][160]) {
  ...
  for (int s = 0; s < SEQ_TILE; ++s) {
    ...
    for (int inner_it = 0; inner_it < FEATURE_DIM_TILE; inner_it += vec_factor) {
      ...
      // aie::mul returns accum; use .to_vector<float>() to keep in fvec_t
