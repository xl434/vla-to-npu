 ...
 *
 * Vectorized GELU for [8][768] per AIE core using Padé rational approximation for tanh.
 * Safe division approach: inv(den), then mul(num, inv) to avoid VRECIP crashes.
 * Max error: < 0.02 vs PyTorch GELU
 */
...
// Padé rational approximation of tanh(x) using safe division.
...
get_tanh_vec(const aie::vector<T_in, vec_factor>& x) {
  ...
  // Safe division: use aie::inv then aie::mul instead of aie::div
...
void gelu_bfloat16(bfloat16 input_x[8][768], bfloat16 output_x[8][768]) {
  event0();
